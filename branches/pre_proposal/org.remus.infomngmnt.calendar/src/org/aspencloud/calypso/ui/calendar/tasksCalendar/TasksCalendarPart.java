...
public class TasksCalendarPart extends BasePart {
	...
	protected void createEditPolicies() {
		installEditPolicy(EditPolicy.LAYOUT_ROLE, new TasksCalendarLayoutEditPolicy()); //new RowEditPolicy());
	}
				...
				if(t0.getStart() != null && t1.getEnd() != null) {
					...
					if(result == 0) {
						if(t0.getStart() != null && t1.getEnd() != null) {
							...
							if(result == 0) {
								Rectangle r0 = (Rectangle) TasksCalendarPart.this.constraints.get(arg0);
								Rectangle r1 = (Rectangle) TasksCalendarPart.this.constraints.get(arg1);
								if((r0 != null) && (r1 != null)) {
		...
		for(Iterator i = getChildren().iterator(); i.hasNext(); ) {
			...
			if(ts1.overlaps(ts2)) {
				if(ts1.getStartDate().after(ts2.getStartDate())
						...
						ts1.getEndDate().before(ts2.getEndDate())) 
						||  
						(ts1.getStartDate().equals(ts2.getStartDate()) &&
						 ts1.getEndDate().equals(ts2.getEndDate()) &&
	...
	public Point getPointFromDate(final Date date) {
		...
		for(int i = 0; i < model.getNumDays(); i++) {
			if((tmpcal.get(Calendar.DAY_OF_YEAR) == model.getDay(i)[1]) &&
				...
				long timeOfDay = 
					(tmpcal.get(Calendar.HOUR_OF_DAY) * 1000*60*60) +
	...
	public long getSnapToDateResolution() {
		return this.snapToDateResolution;
	}

	public void setSnapToDateResolution(final long dateSnapResolution) {
		this.snapToDateResolution = dateSnapResolution;
	}

 ...
 */
package org.remus.infomngmnt.common.core.indexing;
...
[8]   	Nmtokens	   ::=   	Nmtoken (#x20 Nmtoken)*
	...
	private int readNumericEntity() throws IOException {
		...
		try {
			if (ch==';' || ch==-1) {
				this.numWhitespace = this.sb.length() + 2;// + 2 accounts for &, #, and ;, then, take away 1 for the fact that we do output a char
		...
		while(true) {
			...
			case '<':
				...
				if (ret==MATCH) {
					...
					this.numWhitespace = (this.numRead - lastNumRead) - 1;//tack on the -1 since we are returning a space right now
					return ' ';
				}

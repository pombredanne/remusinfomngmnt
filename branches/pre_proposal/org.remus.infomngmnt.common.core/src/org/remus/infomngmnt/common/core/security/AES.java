 ...
 * under the License.
 */
package org.remus.infomngmnt.common.core.security;
...
public class AES {
	 ...
	 */
	public static String getRandomString(final int numBytes) {
	 ...
	 */
	public static byte[] hmacSha1(final byte[] key, final byte[] in)
			throws GeneralSecurityException {
		if (key.length < MIN_HMAC_KEY_LEN) {
			throw new GeneralSecurityException("HMAC key should be at least " + MIN_HMAC_KEY_LEN
					+ " bytes.");
		}
	 ...
	 * @throws GeneralSecurityException
	 */
	public static void hmacSha1Verify(final byte[] key, final byte[] in, final byte[] expected)
			throws GeneralSecurityException {
		Mac hmac = Mac.getInstance(HMAC_TYPE);
		Key hmacKey = new SecretKeySpec(key, HMAC_TYPE);
		hmac.init(hmacKey);
	 ...
	 * @throws GeneralSecurityException
	 */
	public static byte[] aes128cbcEncrypt(final byte[] key, final byte[] plain)
			throws GeneralSecurityException {
	 ...
	 * @throws GeneralSecurityException
	 */
	public static byte[] aes128cbcEncryptWithIV(final byte[] key, final byte[] iv,
			final byte[] plain) throws GeneralSecurityException {
		Cipher cipher = Cipher.getInstance(CIPHER_TYPE);
	 ...
	 * @throws GeneralSecurityException
	 */
	public static byte[] aes128cbcDecrypt(final byte[] key, final byte[] cipherText)
			throws GeneralSecurityException {
	 ...
	 * @throws GeneralSecurityException
	 */
	public static byte[] aes128cbcDecryptWithIv(final byte[] key, final byte[] iv,
			final byte[] cipherText, final int offset) throws GeneralSecurityException {
		Cipher cipher = Cipher.getInstance(CIPHER_TYPE);
		Key cipherKey = new SecretKeySpec(key, CIPHER_KEY_TYPE);
		IvParameterSpec ivSpec = new IvParameterSpec(iv);
	...
	public static void main(final String[] args) {
		try {
			byte[] cipherText = AES.aes128cbcEncrypt("abc1".getBytes(), "Hallo Welt".getBytes());
			byte[] plainText = AES.aes128cbcDecrypt("abc1".getBytes(), cipherText);

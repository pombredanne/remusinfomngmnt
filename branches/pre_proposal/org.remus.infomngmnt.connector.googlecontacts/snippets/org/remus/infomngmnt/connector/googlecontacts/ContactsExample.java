...
public class ContactsExample {
	 ...
	 */
	private static final Logger httpRequestLogger = Logger.getLogger(HttpGDataRequest.class
			.getName());
	...
	public ContactsExample(final ContactsExampleParameters parameters)
		...
		String url = parameters.getBaseUrl() + (parameters.isGroupFeed() ? "groups/" : "contacts/")
				+ parameters.getUserName() + "/" + this.projection;
	 ...
	 */
	private void deleteEntry(final ContactsExampleParameters parameters) throws IOException,
			ServiceException {
	 ...
	 */
	public void updateEntry(final ContactsExampleParameters parameters) throws IOException,
			ServiceException {
		if (parameters.isGroupFeed()) {
	...
	private ContactEntry getContactInternal(final String id) throws IOException, ServiceException {
		return this.service.getEntry(new URL(id.replace("/base/", "/" + this.projection + "/")),
				ContactEntry.class);
	}
	 ...
	 */
	private ContactGroupEntry getGroupInternal(final String id) throws IOException,
			ServiceException {
		return this.service.getEntry(new URL(id.replace("/base/", "/" + this.projection + "/")),
	...
	private static void printContact(final ContactEntry contact) {
		...
		Link photoLink = contact.getLink("http://schemas.google.com/contacts/2008/rel#photo",
				"image/*");
	...
	private static void printGroup(final ContactGroupEntry groupEntry) {
		...
		if (groupEntry.hasSystemGroup()) {
			SystemGroup systemGroup = SystemGroup.fromSystemGroupId(groupEntry.getSystemGroup()
					.getId());
		...
		if (!groupEntry.hasSystemGroup()) {
			// System groups are not modifiable, and thus don't have an edit
			// link.
	 ...
	 */
	private static void processScript(final ContactsExample example,
			final ContactsExampleParameters parameters) throws IOException, ServiceException {
		BufferedReader reader = new BufferedReader(new FileReader(parameters.getScript()));
		try {
			String line;
			while ((line = reader.readLine()) != null) {
				ContactsExampleParameters newParams = new ContactsExampleParameters(parameters,
						line);
	 ...
	 */
	private static void processAction(final ContactsExample example,
			final ContactsExampleParameters parameters) throws IOException, ServiceException {
	 ...
	 */
	private void queryEntries(final ContactsExampleParameters parameters) throws IOException,
			ServiceException {
		...
		if (parameters.getRequireAllDeleted() != null) {
			myQuery
					.setStringCustomParameter("requirealldeleted", parameters
							.getRequireAllDeleted());
		}
	 ...
	 * @param parameters
	 */
	private void listEntries(final ContactsExampleParameters parameters) throws IOException,
			ServiceException {
		if (parameters.isGroupFeed()) {
			...
			for (ContactEntry entry : resultFeed.getEntries()) {
				...
				Link photoLink = entry.getLink("http://schemas.google.com/contacts/2008/rel#photo",
						"image/*");
	 ...
	 */
	private void addEntry(final ContactsExampleParameters parameters) throws IOException,
			ServiceException {
		if (parameters.isGroupFeed()) {
			ContactGroupEntry addedGroup = this.service
					.insert(this.feedUrl, buildGroup(parameters));
			...
			// Store id of the added contact so that scripts can use it in next
			// steps

...
public class TrayHandler extends AbstractHandler {
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		ServiceReference serviceReference = Platform.getBundle(Activator.PLUGIN_ID)
				.getBundleContext().getServiceReference(ITrayService.class.getName());
		if (serviceReference == null
				|| Platform.getBundle(Activator.PLUGIN_ID).getBundleContext().getService(
						serviceReference) == null) {
			Platform.getBundle(Activator.PLUGIN_ID).getBundleContext().registerService(
					ITrayService.class.getName(), new IDETrayService(), null);
		}
		serviceReference = Platform.getBundle(Activator.PLUGIN_ID).getBundleContext()
				.getServiceReference(ITrayService.class.getName());
		ITrayService trayService = (ITrayService) Platform.getBundle(Activator.PLUGIN_ID)

		...
		while (offset < hashable.length) {
			...
			case 'C':
				if (((offset > 1)
						...
						&& (hashable[offset + 2] != 'I') && (hashable[offset + 2] != 'E'))
				...
				if (hasOneOf(meta05, hashable, offset, 2)) {
					...
					if (((offset == 0) && hasOneOf(meta07, hashable,
							(offset + 1), 5))
							|| (hasOneOf(meta08, hashable, offset + 1, 3) && !hasOneOf(
									meta09, hashable, 0, 5))) {
						buffer.append('K');
						offset += 2;
						break;
					...
					if (hasOneOf(meta10, hashable, 0, 4)
							...
							|| ((hasOneOf(meta14, hashable, offset - 1, 1) || (offset == 0)) && hasOneOf(
									meta15, hashable, offset + 2, 1))) {
						buffer.append('K');
					} else {
						if (offset > 0) {
			...
			case 'G':
				if (hashable[offset + 1] == 'H') {
					...
					if (((offset > 1) && hasOneOf(meta33, hashable, offset - 2,
							1))

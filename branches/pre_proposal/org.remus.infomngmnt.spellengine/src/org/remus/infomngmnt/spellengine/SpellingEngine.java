...
public abstract class SpellingEngine implements ISpellingEngine {
	...
	protected static class SpellEventListener implements ISpellEventListener {
		 ...
		 *            the document
		 */
		public SpellEventListener(final ISpellingProblemCollector collector,
				final IDocument document) {
			this.fCollector = collector;
			this.fDocument = document;
			this.fProblemsThreshold = PreferenceConstants.getPreferenceStore().getInt(
					PreferenceConstants.SPELLING_PROBLEMS_THRESHOLD);
		}

		/*
		 * @see
		 * org.eclipse.jdt.internal.ui.text.spelling.engine.ISpellEventListener
		 * #handle(org.eclipse.jdt.internal.ui.text.spelling.engine.ISpellEvent)
		 */
		public void handle(final ISpellEvent event) {
			if (isProblemsThresholdReached())
				return;
			this.fProblemCount++;
			this.fCollector.accept(new JavaSpellingProblem(event, this.fDocument));
		}

...
class IDEIdleHelper {
	...
	IDEIdleHelper(IWorkbenchConfigurer aConfigurer) {
		...
		this.handler = new Runnable() {
			public void run() {
				if (!display.isDisposed() && !IDEIdleHelper.this.configurer.getWorkbench().isClosing()) {
					...
					} else if ((start - IDEIdleHelper.this.lastGC) < IDEIdleHelper.this.nextGCInterval) {
		...
		this.idleListener = new Listener() {
			public void handleEvent(Event event) {
				display.timerExec(IDLE_INTERVAL, IDEIdleHelper.this.handler);
			}
		};
		display.addFilter(SWT.KeyUp, this.idleListener);
		display.addFilter(SWT.MouseUp, this.idleListener);
	}
	...
	private void createGarbageCollectionJob() {
		this.gcJob = new Job(IDEWorkbenchMessages.IDEIdleHelper_backgroundGC) {
			@Override
			protected IStatus run(IProgressMonitor monitor) {
				final Display display = IDEIdleHelper.this.configurer.getWorkbench().getDisplay();
				if (display != null && !display.isDisposed()) {
					...
					if (duration > IDEIdleHelper.this.maxGC) {
						...
						IDEIdleHelper.this.nextGCInterval = Math.max(IDEIdleHelper.this.minGCInterval, GC_DELAY_MULTIPLIER * duration);
						if (Policy.DEBUG_GC) {
							System.out.println("Next GC to run in: " + IDEIdleHelper.this.nextGCInterval); //$NON-NLS-1$
						}
					}
				}
			...
			try {
				display.asyncExec(new Runnable() {
					public void run() {
						...
						display.removeFilter(SWT.KeyUp, IDEIdleHelper.this.idleListener);
						display.removeFilter(SWT.MouseUp, IDEIdleHelper.this.idleListener);
					}
				});

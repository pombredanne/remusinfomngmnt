...
public class TasksCalendarFigure extends ScalableLayeredPane {
	...
	public static final String PROP_TASK_AREA = "taskArea"; //$NON-NLS-1$
	public static final String PROP_DAY_WIDTH = "dayWidth"; //$NON-NLS-1$
	...
	private final int rowMarginWidth = 5;
	private final int borderWidth = 5;
	private final int minorGridDivisions = 2;
	...
	protected void paintFigure(Graphics graphics) {
		...
		int yText;
		int my;
		double rowHeight = (double) (taskArea.height)
				/ (double) (labels.length - 1);
		...
		if (numDays > 0) {
			...
			for (int i = 0; i < numDays; i++) {
				...
				if ((dayColors[i] != null) && !dayColors[i].isDisposed()) {
					...
					graphics.fillRectangle(xLine, taskArea.y,
							(int) dayWidth + 1, taskArea.height);
				}
			}
		}
		...
		for (int i = 0; i < labels.length; i++) {
			...
			if (drawGrid) {
				...
				graphics.drawLine(taskArea.x, yLine, taskArea.x
						+ taskArea.width, yLine);
				...
				if (drawMinorGrid) {
					if (i < labels.length - 1) {
						...
						for (int j = 0; j < (minorGridDivisions - 1); j++) {
							graphics.drawLine(taskArea.x, my, taskArea.x
									+ taskArea.width, my);
			...
			graphics.drawString(labels[i], taskArea.x - d - rowMarginWidth,
					yText);
		}
		...
		if (drawGrid && (numDays > 0)) {
			...
			for (int i = 0; i < numDays; i++) {
				xLine = (int) (taskArea.x + (i * dayWidth));
				graphics.setForegroundColor(ColorConstants.lightGray);
				graphics.drawLine(xLine, taskArea.y, xLine, taskArea.y
						+ taskArea.height);
			}
		}
		...
		graphics.drawLine(taskArea.x - 1, taskArea.y, taskArea.x - 1,
				taskArea.y + taskArea.height);
		if (labels.length == 0) {
			graphics.drawLine(taskArea.x, taskArea.y + 1, taskArea.x
					+ taskArea.width, taskArea.y + 1);
		}
	}
	...
	public void setLabelSize() {
		if (labels.length > 0) {
			for (int i = 0; i < labels.length; i++) {
				Dimension d = FigureUtilities.getStringExtents(labels[i],
						getFont());
				lblWidth = Math.max(lblWidthHint, d.width + 2 * rowMarginWidth
						+ 2 * borderWidth);
	...
	private void updateDimensions() {
		...
		taskArea = new Rectangle(lblWidth, lblHeight / 2 - 1,

...
public class TasksCalendarPart extends BasePart {
	...
	private long snapToDateResolution = 1000 * 60 * 15; // default: 15 minutes
	private final HashMap constraints = new HashMap();

	@Override
	protected IFigure createFigure() {
		TasksCalendarFigure fig = (TasksCalendarFigure) FigureFactory
				.createTasksCalendarFigure();
	...
	protected void createEditPolicies() {
		installEditPolicy(EditPolicy.LAYOUT_ROLE,
				new TasksCalendarLayoutEditPolicy()); // new RowEditPolicy());
	}
	...
	public void propertyChange(final PropertyChangeEvent evt) {
		...
		if (TasksCalendarModel.PROP_DAY.equals(prop)) {
			...
			refreshChildren();
			setTaskConstraints();
		} else if (BaseModel.PROP_SIZE.equals(prop)
				|| BaseModel.PROP_LOCATION.equals(prop)) {
		...
		TreeSet set = new TreeSet(new Comparator() {
			public int compare(final Object arg0, final Object arg1) {
				...
				if (t0.getStart() != null && t1.getEnd() != null) {
					result = t0.getStart().getDate()
							.compareTo(t1.getStart().getDate());
					if (result == 0) {
						if (t0.getStart() != null && t1.getEnd() != null) {
							result = t1.getEnd().getDate()
									.compareTo(t0.getEnd().getDate());
							if (result == 0) {
								Rectangle r0 = (Rectangle) constraints
										.get(arg0);
								Rectangle r1 = (Rectangle) constraints
										.get(arg1);
								if ((r0 != null) && (r1 != null)) {
									result = new Integer(r0.x)
											.compareTo(new Integer(r1.x));
									if ((result == 0) && !r0.isEmpty()
											&& !r1.isEmpty()) {
										System.out
												.println("A Task may be Hidden..."); //$NON-NLS-1$
										System.out
												.println("r0: " + r0.x + ", " + r0.y + ", " + r0.width + ", " + r0.height); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
										System.out
												.println("r1: " + r1.x + ", " + r1.y + ", " + r1.width + ", " + r1.height); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
									}
								}
							}
						}
	...
	private Rectangle getConstraint(final TaskPart part,
			final List requesterList) {
		...
		for (Iterator i = getChildren().iterator(); i.hasNext();) {
			...
			if (ts1.overlaps(ts2)) {
				if (ts1.getStartDate().after(ts2.getStartDate())
						|| (ts1.getStartDate().equals(ts2.getStartDate()) && ts1
								.getEndDate().before(ts2.getEndDate()))
						|| (ts1.getStartDate().equals(ts2.getStartDate())
								&& ts1.getEndDate().equals(ts2.getEndDate()) && !requesterList
								.contains(part2))) {
	...
	public Date getDateFromPoint(final Point point) {
		...
		if (fig.getTaskArea().contains(point)) {
			...
			if ((day >= 0) && (day < model.getNumDays())) {
				...
				double per = (double) (point.y - fig.getTaskArea().y)
						/ (double) (fig.getTaskArea().height - 1);
				long timeOffset = (long) (per * (model.getDayIntervals()[3] - model
						.getDayIntervals()[0]));

				tmpcal.setTimeInMillis(tmpcal.getTimeInMillis() + timeOffset
						+ model.getDayIntervals()[0]);
	...
	public Point getPointFromDate(final Date date) {
		...
		for (int i = 0; i < model.getNumDays(); i++) {
			if ((tmpcal.get(Calendar.DAY_OF_YEAR) == model.getDay(i)[1])
				...
				long timeOfDay = (tmpcal.get(Calendar.HOUR_OF_DAY) * 1000 * 60 * 60)
						+ (tmpcal.get(Calendar.MINUTE) * 1000 * 60)
						+ (tmpcal.get(Calendar.SECOND) * 1000)
						+ tmpcal.get(Calendar.MILLISECOND);
				double per = (double) (timeOfDay - model.getDayIntervals()[0])
						/ (double) (model.getDayIntervals()[3] - model
								.getDayIntervals()[0]);

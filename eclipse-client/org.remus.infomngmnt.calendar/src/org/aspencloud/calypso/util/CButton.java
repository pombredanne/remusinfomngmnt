...
/**
 * A CButton is a class wich wraps an SWT Button to create a widget that acts as
 * much like a native Button as possible while adding the following features:
 * <ul>
...
public class CButton extends Composite {
	...
	public static final int[] Points_Cancel = { 0, 1, 3, 4, 0, 7, 1, 8, 4, 5,
			7, 8, 8, 7, 5, 4, 8, 1, 7, 0, 4, 3, 1, 0 };
	...
	public static final int[] Points_Add = { 2, 4, 4, 4, 4, 2, 5, 2, 5, 4, 7,
			4, 7, 5, 5, 5, 5, 7, 4, 7, 4, 5, 2, 5 };
		...
		public void handleEvent(Event event) {
			...
			if (CButton.this.getShell() == ((Control) event.widget).getShell()) {
				if (SWT.MouseMove == event.type) {
					...
					} else if (button.isVisible() && !button.getSelection()
							&& !event.widget.equals(button)) {
	 ...
	 * @param parent
	 * @param style
	 *            the composite will ignore all style bits and be constructed
	 *            with SWT.NONE
	 *            <p>
	 *            the button will only recognize types of either SWT.PUSH or
	 *            SWT.TOGGLE
	 *            <p>
	 *            other styles: SWT.OK draws a check mark - green if fillColor
	 *            is not given SWT.CANCEL draws an "X" - red if fillColor is not
	 *            given SWT.ARROW with either SWT.LEFT or SWT.RIGHT draws an
	 *            arrow (duh)
	 *            </p>
	 */
	...
	public CButton(Composite parent, int style, Color fillColor) {
		...
		button = new Button(this, bStyle
				| (win32 ? SWT.DOUBLE_BUFFERED : SWT.NONE)); // GTK does not
																// seem to need
																// it...
		...
		if ((style & SWT.OK) != 0) {
			setPolygon(Points_OK,
					(fillColor != null) ? fillColor : (fillColor = getDisplay()
			...
			if ((style & SWT.DOWN) != 0) {
				setPolygon(Points_Down, (fillColor != null) ? fillColor
						: getForeground());
			} else if ((style & SWT.LEFT) != 0) {
				setPolygon(Points_Left, (fillColor != null) ? fillColor
						: getForeground());
			} else if ((style & SWT.RIGHT) != 0) {
				setPolygon(Points_Right, (fillColor != null) ? fillColor
						: getForeground());
			} else if ((style & SWT.UP) != 0) {
				setPolygon(Points_Up, (fillColor != null) ? fillColor
						: getForeground());
			}
		} else if ((style & SWT.UP) != 0) {
			setPolygon(Points_Add, (fillColor != null) ? fillColor
					: getForeground());
		} else if ((style & SWT.DOWN) != 0) {
			setPolygon(Points_Subtract, (fillColor != null) ? fillColor
					: getForeground());
		}
	...
	private void drawControl(Event e) {
		if (text != null) {
			...
			if (e.widget instanceof Button) {
				...
				GC gc = new GC(img);
				gc.drawImage(image, -(size.width - r.width) / 2 - marginLeft
						- ix, -(size.height - r.height) / 2 - marginTop - iy);
				e.gc.drawImage(img, marginLeft, marginTop);
				gc.dispose();
				img.dispose();
			} else {
				e.gc.drawImage(image, (r.width - size.width) / 2 - ix,
	...
	/**
	 * if param equal is true, the x and y sizes of this CButton will be forced

...
public class AnnotatingQuickFixTextBox implements ModifyListener, TraverseListener, FocusListener,
	...
	public AnnotatingQuickFixTextBox(final Composite composite, final String message,
		...
		final IHandlerActivation handlerActivation = installQuickFixActionHandler(handlerService,
				sourceViewer);
		...
		quickFixMenu.addMenuListener(new IMenuListener() {
			...
			public void menuAboutToShow(final IMenuManager manager) {
				...
				while (annotationIterator.hasNext()) {
					org.eclipse.jface.text.source.Annotation annotation = (org.eclipse.jface.text.source.Annotation) annotationIterator
							.next();
					if (!annotation.isMarkedDeleted()
							&& includes(annotationModel.getPosition(annotation), sourceViewer
						...
						for (int i = 0; i < computeQuickAssistProposals.length; i++) {
							...
							quickFixMenu.add(new Action(proposal.getDisplayString()) {
								 ...
								 * (non-Javadoc)
								 * 
								 * @seeorg.eclipse.jface.action.Action#
								 * getImageDescriptor()
								 */
								@Override
		...
		this.fTextField.addFocusListener(new FocusListener() {
			...
			public void focusGained(final FocusEvent e) {
				...
				this.cutHandlerActivation = service.activateHandler(
						IWorkbenchActionDefinitionIds.CUT, new ActionHandler(cutAction),
				...
				this.pasteHandlerActivation = service.activateHandler(
						IWorkbenchActionDefinitionIds.PASTE, new ActionHandler(pasteAction),
						new ActiveShellExpression(composite.getShell()));
				this.selectAllHandlerActivation = service.activateHandler(
						IWorkbenchActionDefinitionIds.SELECT_ALL,
						new ActionHandler(selectAllAction), new ActiveShellExpression(composite
								.getShell()));
	 ...
	 * @since 3.4
	 */
	private IHandlerActivation installQuickFixActionHandler(final IHandlerService handlerService,
			final SourceViewer sourceViewer) {
		return handlerService.activateHandler(ITextEditorActionDefinitionIds.QUICK_ASSIST,
				createQuickFixActionHandler(sourceViewer), new ActiveShellExpression(sourceViewer
	...
	protected void firePropertyChangeChange(final String property, final Object oldValue,

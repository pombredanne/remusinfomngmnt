    ...
    public void run() {
        for (String key : formatMappings.keySet()) {
            if (formatMappings.get(key).equals(this.target.getText())) {
                this.composer.execute(JavaScriptCommands.FORMAT("<" + key + ">")); //$NON-NLS-1$ //$NON-NLS-2$
                break;
            }
        }
    }
    ...
    public void handleEvent(Event event) {
        ...
        if (event.type != EventConstants.ALL && !ComposerStatus.UNDEFINED.equals(evtProps.getProperty(PropertyConstants.VALUE))) {
            for (int i = 0,n = this.target.getItems().length; i < n; i++) {
                if (this.target.getItem(i).equals(formatMappings.get(evtProps.getProperty(PropertyConstants.VALUE)))) {

...
public class TrustedAuthenticator extends Authenticator {
	...
	public TrustedAuthenticator(Map config) throws IOException, OAuthException,
		...
		if (config.get("access_token_key") != null) {
			...
			consumer.setTokenWithSecret(
					(String) config.get("access_token_key"),
					(String) config.get("access_token_secret"));
		}

	}

	@SuppressWarnings("unchecked")
	public boolean retrieveTrustedAccessToken(String user_name,
			String user_password) throws OAuthCommunicationException,
			OAuthMessageSignerException, IOException, ParseException,
			OAuthExpectationFailedException, DropboxException {
		...
		OAuthConsumer cons = new CommonsHttpOAuthConsumer(consumer_key,
				consumer_secret);

		String target = RESTUtility.buildFullURL(RESTUtility.secureProtocol(),
				api_host, port,
				RESTUtility.buildURL("/token", API_VERSION, params));
		...
		try {
			...
			consumer.setTokenWithSecret((String) creds.get("token"),
					(String) creds.get("secret"));

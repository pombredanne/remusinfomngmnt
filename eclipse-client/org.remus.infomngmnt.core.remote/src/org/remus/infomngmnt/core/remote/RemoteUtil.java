...
/**
 * Utility class for {@link RemoteRepository}, {@link RemoteContainer} and
 * {@link RemoteObject}. While synchronizing with remote Information end-points
 * it's necessary to prepare some objects. This class provides some helper
 * methods which are used in different contexts.
 * 
...
public class RemoteUtil {
	...
	/**
	 * Searches in the object hierarchy for the {@link RemoteRepository} from
	 * which the {@link RemoteObject} comes from.
	 * 
	 * @param object
	 *            the object
	 * @return the local repository definition if present, else
	 *         <code>null</code>.
	 * @since 1.0
	 */
	public static RemoteRepository getRemoteRepository(final RemoteObject object) {
		...
		} else if (object.eContainer() != null && object.eContainer() instanceof RemoteObject) {
	 ...
	 * @return
	 */
	public static Map<RemoteRepository, List<Adapter>> groupByRemoteRepsoitory(
			final List<Adapter> list) {
		...
		for (Adapter object : list) {
			if (object.getAdapter(SynchronizationMetadata.class) != null) {
				SynchronizationMetadata metadata = (SynchronizationMetadata) object
						.getAdapter(SynchronizationMetadata.class);
				String repositoryId = metadata.getRepositoryId();
				RemoteRepository localRepository = RemoteActivator.getDefault().getServiceTracker()
		...
		for (final RemoteRepository remoteRepository : keySet) {
			List<Adapter> filteredList = CollectionUtils.filter(returnValue.get(remoteRepository),
					new CollectionFilter<Adapter>() {
						public boolean select(final Adapter item) {
							return !ModelUtil.containsParent(returnValue.get(remoteRepository),
									item)

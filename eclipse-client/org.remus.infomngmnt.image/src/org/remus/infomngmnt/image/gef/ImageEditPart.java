 ...
 */
class ImageEditPart extends AbstractGraphicalEditPart 
implements Adapter, IEditingDomainHolder  {
	...
	public void activate() {
		if (!isActive()) {
			super.activate();
			InformationUnit unit = InformationUtil.getChildByType(getCastedModel(), ImagePlugin.NODE_NAME_LINKS);
	...
	protected IFigure createFigure() {
		final InformationUnit unit = InformationUtil.getChildByType(getCastedModel(), ImagePlugin.NODE_NAME_RAWDATA);
		this.width = InformationUtil.getChildByType(getCastedModel(), ImagePlugin.NODE_NAME_WIDTH).getLongValue();
		this.height = InformationUtil.getChildByType(getCastedModel(), ImagePlugin.NODE_NAME_HEIGHT).getLongValue();
		ByteArrayInputStream bis = new ByteArrayInputStream(unit.getBinaryValue());
	...
	protected List getModelChildren() {
		...
		InformationUnit unit = InformationUtil.getChildByType(getCastedModel(), ImagePlugin.NODE_NAME_LINKS);
		EList<InformationUnit> childValues = unit.getChildValues(); // return a list of shapes
		for (InformationUnit informationUnit : childValues) {
			returnValue.add(new ShapableInfoDelegate(
					informationUnit, 
		...
		protected Command createChangeConstraintCommand(final ChangeBoundsRequest request,
			...
			if (child instanceof CommentEditPart && constraint instanceof Rectangle) {
				...
				return new ShapeSetConstraintCommand(
						(ShapableInfoDelegate) child.getModel(), request, (Rectangle) constraint);
			}

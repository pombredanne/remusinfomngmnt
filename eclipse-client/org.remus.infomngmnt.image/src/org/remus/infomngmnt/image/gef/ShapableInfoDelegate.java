...
public class ShapableInfoDelegate extends AdapterImpl{
	...
	public ShapableInfoDelegate(final InformationUnit unit, final Dimension imageDimensions, final EditingDomain domain) {
	...
	public Dimension getSize() {
		...
		return new Dimension(
				(int) Math.round((double)this.imageDimensions.width * (int)diwWidth.getLongValue() / 1000),
				(int) Math.round((double)this.imageDimensions.height * (int)diwHeight.getLongValue() / 1000));
	}
	...
	public Point getLocation() {
		...
		return new Point(
				Math.round((double)this.imageDimensions.width * (int)locX.getLongValue() / 1000),
				Math.round((double)this.imageDimensions.height * (int)locY.getLongValue() / 1000));
	}
	...
	public void setSize(final Dimension size) {
		...
		cc.append(new SetCommand(
				...
				Math.round((double)size.width / this.imageDimensions.width * 1000)));
		cc.append(new SetCommand(
				this.domain,
				diwHeight,
				InfomngmntPackage.Literals.INFORMATION_UNIT__LONG_VALUE,
				Math.round((double)size.height / this.imageDimensions.height * 1000)));
	...
	public void setLocation(final Point location) {
		...
		cc.append(new SetCommand(
				...
				locX,
				InfomngmntPackage.Literals.INFORMATION_UNIT__LONG_VALUE,
				Math.round((double)location.x / this.imageDimensions.width * 1000)));
		cc.append(new SetCommand(
				this.domain,
				locY,
				InfomngmntPackage.Literals.INFORMATION_UNIT__LONG_VALUE,
				Math.round((double)location.y / this.imageDimensions.height * 1000)));

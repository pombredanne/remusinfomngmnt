...
public class DefaultLuceneCustomizer implements ILuceneCustomizer {
	...
	public Document getLuceneDocument(final InformationUnit document, final IProject project,
		...
		try {
			Field titleField = new Field(LuceneSearchService.SEARCHINDEX_LABEL,
					checkNull(informationRepresentation.getTitleForIndexing(monitor)),
		...
		try {
			...
			Field contentField = new Field(LuceneSearchService.SEARCHINDEX_CONTENT,
					checkNull(informationRepresentation.getBodyForIndexing(monitor)),
					Field.Store.YES, Field.Index.TOKENIZED);
		...
		try {
			Field additionalField = new Field(LuceneSearchService.SEARCHINDEX_ADDITIONALS,
					checkNull(informationRepresentation.getAdditionalsForIndexing(monitor)),
					Field.Store.YES, Field.Index.TOKENIZED);
		...
		Field descriptionField = new Field(LuceneSearchService.SEARCHINDEX_DESCRIPTION,
		...
		Field keywordField = new Field(LuceneSearchService.SEARCHINDEX_KEYWORDS, checkNull(document
		...
		if (search.getSearchString() != null) {
			if (search.isIdSearch()) {
				...
				for (String string : split) {
					fieldList.add(LuceneSearchService.SEARCHINDEX_ITEM_ID);
					termList.add(string);

...
public class SearchView extends AbstractScrolledTitledView {
	...
	public void createViewContents(final Composite parent) {
		...
		hyperlink.addHyperlinkListener(new HyperlinkAdapter() {
			...
			public void linkActivated(final HyperlinkEvent e) {
				Program.launch(SearchUIActivator.getDefault().getPreferenceStore().getString(
		...
		this.applicationModel.getModel().eAdapters().add(this.rootCatAdapter);
	...
	protected void handleSearchButtonPressed() {
		...
		this.searchService.search(this.currentSearch, true, new ISearchCallBack() {
					...
					public void run() {
						try {
							...
							PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage()
									.openEditor(new URIEditorInput(search.eResource().getURI()),
	...
	@Override
	public void dispose() {
		this.applicationModel.getModel().eAdapters().remove(this.rootCatAdapter);
		this.ctx.dispose();
		super.dispose();
	}
	...
	private void initDataBinding() {
		...
		DatePickerBindingWidget dateComboBinding = AdditionalBindingWidgetFactory
				.createDateComboBinding(this.fromDate, this.ctx, this.editingDomain);
		...
		DatePickerBindingWidget dateComboBinding2 = AdditionalBindingWidgetFactory
				.createDateComboBinding(this.toDate, this.ctx, this.editingDomain);
	...
	private void initCheckListInput() {
		if (!this.newCheckList.getControl().isDisposed()) {

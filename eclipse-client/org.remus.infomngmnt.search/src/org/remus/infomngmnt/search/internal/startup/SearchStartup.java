...
package org.remus.infomngmnt.search.internal.startup;
...
/**
 * <p>
 * The local startup of the search plugin loads the latest searches and deletes
 * old files.
 * </p>
 * <p>
...
public class SearchStartup {
	...
	public void earlyStartup() {
		...
		// FIXME
		int int1 = 20;
		File file = SearchActivator.getDefault().getStateLocation().append("localsearches")
				.toFile();
		if (!file.exists()) {
			file.mkdirs();
		}
		...
		for (int i = 0; i < counter; i++) {
			Search search = handler.getObjectFromUri(new Path(listFilesCollection.get(i)
					.getAbsolutePath()), SearchPackage.Literals.SEARCH);
			SearchActivator.getDefault().getSearchHistory().getSearch().add(search);
		}

 ...
 * @author Tom Seidel
 */
public abstract class AbstractDesktopWindow extends Window {
	...
	private final Job closeJob = new Job(LABEL_JOB_CLOSE) {
		...
		protected IStatus run(final IProgressMonitor monitor) {
			if (!AbstractDesktopWindow.this.display.isDisposed()) {
				AbstractDesktopWindow.this.display.asyncExec(new Runnable() {
					public void run() {
						Shell shell = AbstractDesktopWindow.this.getShell();
						...
						AbstractDesktopWindow.this.closeFade();
					}
		...
		public void handleEvent(final Event event) {
			if (isMouseOver(getShell())) {
				AbstractDesktopWindow.this.fadeJob = SwtFadeUtil.fastFadeIn(
						AbstractDesktopWindow.this.shell, new IFadeListener() {
	...
	protected void createTitleArea(final Composite parent) {
		...
		titleTextLabel.addMouseListener(new MouseAdapter() {
			...
			public void mouseDown(final MouseEvent e) {
				AbstractDesktopWindow.this.xOffset = e.x;
				AbstractDesktopWindow.this.yOffset = e.y;
				AbstractDesktopWindow.this.moveMode = true;
			}

			@Override
			public void mouseUp(final MouseEvent e) {
				AbstractDesktopWindow.this.moveMode = false;
			}
		});
		...
		restore.setImage(this.commonImages.getRestore());
		restore.addMouseTrackListener(new MouseTrackAdapter() {
			@Override
			public void mouseEnter(final MouseEvent e) {
				restore.setImage(AbstractDesktopWindow.this.commonImages.getRestoreHover());
			}

			@Override
			public void mouseExit(final MouseEvent e) {
				restore.setImage(AbstractDesktopWindow.this.commonImages.getRestore());
			}
		});
		...
		button.setImage(this.commonImages.getClose());
		button.addMouseTrackListener(new MouseTrackAdapter() {
			@Override
			public void mouseEnter(final MouseEvent e) {
				button.setImage(AbstractDesktopWindow.this.commonImages.getCloseHover());
			}

			@Override
			public void mouseExit(final MouseEvent e) {
				button.setImage(AbstractDesktopWindow.this.commonImages.getClose());
			}
		});
	...
	private void initResources() {
		...
		this.commonImages = new CommonImages(this.resources);
	...
	protected Control createContents(final Composite parent) {
		...
		titleCircle.addControlListener(new ControlAdapter() {
			...
			public void controlResized(final ControlEvent e) {
				...
				titleCircle.setBackgroundImage(AbstractDesktopWindow.this.lastUsedBgImage);
			...
			private void drawGradient(final GC gc, final Rectangle clArea) {
				gc.setForeground(AbstractDesktopWindow.this.color.getGradientBegin());
				gc.setBackground(AbstractDesktopWindow.this.color.getGradientEnd());
				gc.fillGradientRectangle(clArea.x, clArea.y, clArea.width, clArea.height, true);
			}

			private void fixRegion(final GC gc, final Rectangle clArea) {
				gc.setForeground(AbstractDesktopWindow.this.color.getBorder());
		...
		middleContentCircle.addControlListener(new ControlAdapter() {
			...
			public void controlResized(final ControlEvent e) {
				...
				Image image = new Image(middleContentCircle.getDisplay(), clArea.width,
						clArea.height);
				GC gc = new GC(image);
				gc.setForeground(AbstractDesktopWindow.this.color.getBorder());
	...
	public ITrayService getTrayService() {
		final BundleContext bundleContext = Platform.getBundle("org.remus.infomngmnt.ui.popup")
				.getBundleContext();

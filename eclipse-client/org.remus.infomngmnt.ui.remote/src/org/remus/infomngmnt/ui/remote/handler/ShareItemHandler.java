		...
		if (currentSelection instanceof IStructuredSelection) {
			...
			for (Object object : list) {
				if (object instanceof InformationUnitListItem) {
					...
					try {
						IRepositoryExtensionService extensionService = UIPlugin.getDefault()
								.getServiceTracker().getService(IRepositoryExtensionService.class);
						AbstractExtensionRepository repositoryById = extensionService
								.getItemByLocalObject((SynchronizableObject) object);

						RemoteObject addToRepository = repositoryById.addToRepository(
								(InformationUnitListItem) object, null);

...
public class DeleteRepositoryAction extends DeleteAction {
	...
	public void run() {
		...
		if (MessageDialog.openConfirm(UIUtil.getDisplay().getActiveShell(), "Confirm delte",
			...
			for (Object object : list) {
				if (object instanceof RemoteRepository) {
					...
					EList<Category> model = UIPlugin.getDefault().getApplicationService()
							.getModel().getRootCategories();
					...
					try {
						AbstractExtensionRepository itemByRepository = UIPlugin.getDefault()
		...
		super.run();
		RepositoryCollection repositories = UIPlugin.getDefault().getServiceTracker().getService(

...
public abstract class CheckOutMultipeContainerWizard extends MultipleNewObjectsWizard {
	...
	public void init(final IWorkbench workbench, final IStructuredSelection selection) {
		ProgressMonitorDialog pmd = new ProgressMonitorDialog(workbench.getActiveWorkbenchWindow()
				.getShell());
		try {
			pmd.run(true, true, new CancelableRunnable() {
				...
				protected IStatus runCancelableRunnable(final IProgressMonitor monitor) {
					if (UIUtil.isSelectionInstanceOf(selection, RemoteObject.class)) {
						...
						// IRepository itemById =
						// UIPlugin.getDefault().getService(IRepositoryExtensionService.class).getItemById(list.get(0).getRepositoryTypeId());
						// CheckOutMultipeContainerWizard.this.convertedObjects
						// = itemById.convertToLocalObjects(list.toArray(new
						// RemoteObject[list.size()]), monitor);
						// setNewObjects(new
						// BasicEList<InformationUnit>(CheckOutMultipeContainerWizard.this.convertedObjects.keySet()));
					}
	...
	public boolean performFinish() {
		Job job = new Job("Creating new items") {
			...
			protected IStatus run(final IProgressMonitor monitor) {
				monitor.beginTask("Creating new items",
						CheckOutMultipeContainerWizard.this.convertedObjects.size());
				for (InformationUnit newElement : CheckOutMultipeContainerWizard.this.convertedObjects
						.keySet()) {
					UIPlugin.getDefault().getEditService().getNavigationEditingDomain()
							.getCommandStack().execute(
									CommandFactory.CREATE_INFOTYPE(newElement, findCategory()));

					InformationUnitListItem adapter = (InformationUnitListItem) newElement
							.getAdapter(InformationUnitListItem.class);
					if (adapter != null) {
						adapter
								.setSynchronizationMetaData(CheckOutMultipeContainerWizard.this.convertedObjects
										.get(newElement));
					}

...
public class RulePreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	private IRuleExtensionService ruleExtensionService;
	...
	protected Control createContents(final Composite parent) {
		...
		this.ruleExtensionService = this.serviceTracker.getService(IRuleExtensionService.class);
		...
		AdapterFactoryContentProvider ocp = new AdapterFactoryContentProvider(getAdapterFactory());
			...
			public void handleEvent(final Event event) {
				...
				if (diag.open() == IDialogConstants.OK_ID) {
					Command setCommand = SetCommand.create(RulePreferencePage.this.editingDomain,
							...
							RulesPackage.Literals.RULE_ACTION__GROOVY_MATCHER, diag.getScript());
		...
		this.postProcessingActionsButton.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(final Event event) {
				...
				if (diag.open() == IDialogConstants.OK_ID) {
					Command setCommand = SetCommand.create(RulePreferencePage.this.editingDomain,
							RulePreferencePage.this.selectedRuleAction,
							RulesPackage.Literals.RULE_ACTION__POST_PROCESSING_INSTRUCTIONS, diag
									.getScript());
					RulePreferencePage.this.editingDomain.getCommandStack().execute(setCommand);
				}
			}
	...
	protected void handleSelectionChanged(final ISelection selection) {
		...
		if (element instanceof RuleAction) {
			...
			IObservableValue emfName = EMFObservables.observeValue((EObject) element,
					RulesPackage.Literals.RULE_ACTION__NAME);
	...
	public void init(final IWorkbench workbench) {
		this.ruleService = RuleEditPlugin.getPlugin().getServiceTracker().getService(
				IRuleService.class);
		this.model = this.ruleService.getElementRules();
	...
	protected void populateNewMenu(final ISelection selection) {
		if (((IStructuredSelection) selection).getFirstElement() instanceof RemusTransferType) {
			...
			Map<String, List<String>> allTransferTypes = this.ruleExtensionService.getTransfers();
			Set<String> keySet = allTransferTypes.keySet();
			for (String string : keySet) {
				if (allTransferTypes.get(string).contains(element.getId())) {
					IInfoType infoTypeByType = this.infoTypeService.getInfoTypeByType(string);
					if (infoTypeByType != null) {
						this.newMenuManager.add(new NewItemAction(infoTypeByType, element));
					}
				}
			}
		}
	...
	private ComposedAdapterFactory getAdapterFactory() {
		...
		adapterFactory.addAdapterFactory(new RulesAdapterFactory());
			...
			if (createCommand.canExecute()) {
				...
				for (Object object : clipboard) {
					if (object instanceof RuleAction
						...
						IInfoType infoTypeByType = RulePreferencePage.this.infoTypeService
								.getInfoTypeByType(infoTypeId);
						if (infoTypeByType != null
								&& RulePreferencePage.this.ruleExtensionService
										.getTransferIdsByTypeId(infoTypeId)
										.contains(
	...
	public void performApply() {
		synchronized (this.model.eResource()) {
			try {
	...
	private class NewItemAction extends Action {
		...
		public NewItemAction(final IInfoType infoType, final RemusTransferType element) {
			...
			IInformationTypeImage service = RulePreferencePage.this.serviceTracker
					.getService(IInformationTypeImage.class);
			setImageDescriptor(service.getImageDescriptorByInfoType(infoType.getType()));
			setText(infoType.getName());
			// RulePreferencePage.this.serviceTracker.ungetService(service);
		}

		@Override
		public void run() {
			RuleAction createRuleAction = RulesFactory.eINSTANCE.createRuleAction();

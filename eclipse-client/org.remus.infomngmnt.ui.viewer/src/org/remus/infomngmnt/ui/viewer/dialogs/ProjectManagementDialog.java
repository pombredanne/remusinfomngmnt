package org.remus.infomngmnt.ui.viewer.dialogs;
...
public class ProjectManagementDialog extends TitleAreaDialog {
		...
		this.closeButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(final Event event) {
				...
				for (Object object : list) {
					Category itemByValue = (Category) ModelUtil.getItemByValue(
							ProjectManagementDialog.this.applicationService.getModel()
									.getRootCategories(),
					...
					try {
						org.remus.infomngmnt.resources.util.ResourceUtil
								.closeProject((IProject) object);
				...
				if (MessageDialog
					...
					IRunnableWithProgress progress = new IRunnableWithProgress() {
						...
						public void run(final IProgressMonitor monitor)
							...
							for (Object object : list) {
								try {
									org.remus.infomngmnt.resources.util.ResourceUtil.deleteProject(
											(IProject) object, monitor);
								} catch (CoreException e) {
									throw new InvocationTargetException(e);
								}
							}
		...
		this.moveUpButton.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(final Event event) {
				Category c1 = (Category) ModelUtil
						.getItemByValue(
								ViewerActivator.getDefault().getApplicationService().getModel()
										.getRootCategories(),
				...
				int oldIndex = ViewerActivator.getDefault().getApplicationService().getModel()
						.getRootCategories().indexOf(c1);
				ViewerActivator.getDefault().getApplicationService().getModel().getRootCategories()
						.move(oldIndex - 1, oldIndex);
				ProjectManagementDialog.this.tableViewer.refresh();
			}
	...
	private boolean checkMoveUp(final IStructuredSelection selection) {
		int index = ViewerActivator.getDefault().getApplicationService().getModel()
				.getRootCategories().indexOf(
						ModelUtil.getItemByValue(ViewerActivator.getDefault()
								.getApplicationService().getModel().getRootCategories(),
								InfomngmntPackage.Literals.CATEGORY__LABEL, ((IProject) selection
	...
	private boolean checkMoveDown(final IStructuredSelection selection) {
		int index = this.applicationService.getModel().getRootCategories().indexOf(
				ModelUtil.getItemByValue(this.applicationService.getModel().getRootCategories(),
						InfomngmntPackage.Literals.CATEGORY__LABEL, ((IProject) selection
								.getFirstElement()).getName()));
		return (index + 1) < this.applicationService.getModel().getRootCategories().size()
	...
	public boolean close() {
		UIUtil.saveDialogSettings(ViewerActivator.getDefault(), this);
		return super.close();
	}

	private void bindValuesToUI() {
		this.tableViewer.setContentProvider(ArrayContentProvider.getInstance());
		this.tableViewer.setInput(org.remus.infomngmnt.resources.util.ResourceUtil
				.getRelevantAndClosedProjects());
		...
		this.tableViewer.setComparator(new ViewerComparator() {
			...
			public int compare(final Viewer viewer, final Object e1, final Object e2) {
				...
				if (p1.isOpen() && p2.isOpen()) {
					Category c1 = (Category) ModelUtil.getItemByValue(ViewerActivator.getDefault()
							.getApplicationService().getModel().getRootCategories(),
							InfomngmntPackage.Literals.CATEGORY__LABEL, p1.getName());
					Category c2 = (Category) ModelUtil.getItemByValue(ViewerActivator.getDefault()
							.getApplicationService().getModel().getRootCategories(),
							InfomngmntPackage.Literals.CATEGORY__LABEL, p2.getName());

					int i1 = ViewerActivator.getDefault().getApplicationService().getModel()
							.getRootCategories().indexOf(c1);
					int i2 = ViewerActivator.getDefault().getApplicationService().getModel()
							.getRootCategories().indexOf(c2);
	...
	protected Point getInitialSize() {
		return UIUtil.getDialogSettingsInitialSize(ViewerActivator.getDefault(), this, new Point(
				500, 375));
	}

	@Override
	protected Point getInitialLocation(final Point initialSize) {
		return UIUtil.getDialogSettingsInitialLocation(ViewerActivator.getDefault(), this, super

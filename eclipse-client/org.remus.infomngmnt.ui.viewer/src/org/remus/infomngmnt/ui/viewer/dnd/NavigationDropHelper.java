...
public class NavigationDropHelper {
	...
	public static Command checkProjectRelevance(final Collection<?> source, final Object target,
		...
		for (InformationUnitListItem informationUnitListItem : relevantItems) {
			...
			if (copy) {
				compoundCommand.append(SetCommand.create(ViewerActivator.getDefault()
						.getEditService().getNavigationEditingDomain(), informationUnitListItem,
			...
			Command setWorkspacePathCommand = CommandFactory.SET_WORKSPACEPATH(
					informationUnitListItem, targetPath, ViewerActivator.getDefault()
							.getEditService().getNavigationEditingDomain());
				...
				if (syncMetaData != null) {
					...
					if (syncMetaData.getSyncState() != SynchronizationState.NOT_ADDED
						...
						if (newReference != null) {
							compoundCommand.append(new CreateChildCommand(ViewerActivator
									.getDefault().getEditService().getNavigationEditingDomain(),
									((EObject) object).eContainer(), newReference, newObject,
					...
					if (targetSyncData == null) {
						...
						if (((IAdaptable) ((EObject) object).eContainer())
							...
							compoundCommand.append(CommandFactory.REMOVE_SYNCDATACOMMAND(
									(SynchronizableObject) object, ViewerActivator.getDefault()
											.getEditService().getNavigationEditingDomain()));
							...
							while (eAllContents.hasNext()) {
								...
								if (eObject instanceof SynchronizableObject) {
									compoundCommand.append(CommandFactory.REMOVE_SYNCDATACOMMAND(
											(SynchronizableObject) eObject, ViewerActivator
													.getDefault().getEditService()
						...
						while (eAllContents.hasNext()) {
							...
							if (eObject instanceof SynchronizableObject) {
								...
								compoundCommand.append(SetCommand.create(
										ViewerActivator.getDefault().getEditService()
												.getNavigationEditingDomain(),
										((SynchronizableObject) eObject)
												.getSynchronizationMetaData(),

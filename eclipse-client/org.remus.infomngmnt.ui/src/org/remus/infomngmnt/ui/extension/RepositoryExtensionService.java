...
public class RepositoryExtensionService extends PluginRegistryDynamic implements IRepositoryExtensionService {
	...
	public void init() {
		...
		for (final IConfigurationElement configurationElement : configurationElements) {
			IRepositoryUI item = new RepositoryUI(
					configurationElement,
					configurationElement.getAttribute(ID_ATT),
					configurationElement.getAttribute(REPOSITORY_ID));
			this.items.put(item.getId(), item);
		}
	}
	
	/* (non-Javadoc)
	 * @see org.remus.infomngmnt.ui.service.IRepositoryExtensionService#getAllItems()
	 */

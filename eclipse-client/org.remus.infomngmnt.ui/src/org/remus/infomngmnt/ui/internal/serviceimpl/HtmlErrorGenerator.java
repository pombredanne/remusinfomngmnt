...
public class HtmlErrorGenerator implements IHtmlGenerationErrorGenerator {
	...
	public InputStream buildErrorRepresentation(final InformationUnit unit, final Exception e) {
		...
		try {
			InputStream templateIs = FileLocator.openStream(Platform.getBundle(UIPlugin.PLUGIN_ID),
			...
			FreemarkerRenderer.getInstance().process(UIPlugin.PLUGIN_ID, templateIs, returnValue,
					freemarkParameters, read.getContentsAsStrucuturedMap(),

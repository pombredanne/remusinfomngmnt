...
import org.remus.infomngmnt.ui.rules.quickaccess.NewElementQuickAccessProider;
...
public class DropSection extends AbstractTraySection {
	...
	public void init(final FormToolkit pToolkit, final TraySection section) {
		...
		this.ruleByName = UimodelEditPlugin.getPlugin().getServiceTracker().getService(
				IRuleService.class).getRuleByName(string);
	}
	...
	public void createDetailsPart(final Composite parent) {
		...
		Composite dropComposite = this.toolkit.createComposite(parent, SWT.BORDER | SWT.BORDER_DOT
				| SWT.NO_FOCUS);
		...
		dropTarget.addDropListener(new DropTargetListener() {
			...
			public void drop(final DropTargetEvent event) {
				event.widget.getDisplay().asyncExec(new Runnable() {
					public void run() {
						List<RuleResult> process = RuleProcessor.getInstance().process(event,
								DropSection.this.ruleByName);
	...
	private Transfer[] getTransferTypes() {
		...
		Collection<TransferWrapper> values = UimodelEditPlugin.getPlugin().getServiceTracker()
				.getService(IRuleExtensionService.class).getAllTransferTypes().values();

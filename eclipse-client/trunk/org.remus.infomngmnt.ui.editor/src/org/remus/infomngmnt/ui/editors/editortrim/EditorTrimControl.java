...
public class EditorTrimControl extends Composite {
	...
	private final IEditorExtensionService editorTrimService;
	...
	public EditorTrimControl(final Composite parent, final int style) {
		...
		this.editorTrimService = EditorActivator.getDefault().getServiceTracker().getService(
	...
	public void buildHyperLink(final String typeId) {
		String perspectiveId = getInformationPerspectiveId();
		String commandId = this.editorTrimService.getEditorTrimCommandIdByTypeId(typeId);
		if (commandId == null
				&& perspectiveId != null
				&& !perspectiveId.equals(PlatformUI.getWorkbench().getActiveWorkbenchWindow()
	...
	public void buildBackButton(final String infoId) {
		this.itemById = EditorActivator.getDefault().getApplicationService().getItemById(infoId,
	...
	private void executeBack(final InformationUnitListItem item) {
		String informationPerspectiveId = getInformationPerspectiveId();
		if (informationPerspectiveId != null) {
			final IPerspectiveDescriptor perspectiveDescriptor = PlatformUI.getWorkbench()
					.getPerspectiveRegistry().findPerspectiveWithId(informationPerspectiveId);
	...
	public String getInformationPerspectiveId() {
		final BundleContext bundleContext = EditorActivator.getDefault().getBundle()
				.getBundleContext();
		final ServiceReference serviceReference = bundleContext
				.getServiceReference(IPerspectiveService.class.getName());
		if (serviceReference != null) {
			IPerspectiveService service = (IPerspectiveService) bundleContext

...
public class RemoteRepositoryDeferredAdapter implements IDeferredWorkbenchAdapter {
	...
	private IRepository itemById;

	public RemoteRepositoryDeferredAdapter(final RemoteObject object) {
		this.remoteObject = object;
		IRepositoryExtensionService extensionService = UIPlugin.getDefault().getServiceTracker()
				.getService(IRepositoryExtensionService.class);
		try {
			this.itemById = extensionService.getItemByRepository(getRepository(this.remoteObject));

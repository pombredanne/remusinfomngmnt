...
public class RepositoryUI implements IRepositoryUI {
	...
	public NewRepositoryWizard getWizardClass() {
		try {
			this.wizardClass = (NewRepositoryWizard) this.configurationElement
					.createExecutableExtension(RepositoryExtensionService.WIZARD_CLASSID);
	...
	public IRepositoryActionContributor getActionContributor() {
		if (this.actionContributor == null) {
			try {
				this.actionContributor = (IRepositoryActionContributor) this.configurationElement
						.createExecutableExtension(RepositoryExtensionService.ACTIONCONTRIBUTOR_CLASSID);
			} catch (CoreException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}

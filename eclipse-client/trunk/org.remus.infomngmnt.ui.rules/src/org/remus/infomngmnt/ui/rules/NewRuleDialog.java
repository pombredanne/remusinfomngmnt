...
public class NewRuleDialog extends StatusDialog {
	 ...
	 * @param parentShell
	 */
	public NewRuleDialog(final Shell parentShell, final AvailableRuleDefinitions model) {
		super(parentShell);
		this.model = model;
		this.newElement = RulesFactory.eINSTANCE.createNewElementRules();
	...
	private void bindValuesToUi() {
		...
		IObservableValue emfName = EMFObservables.observeValue(this.newElement,
	...
	protected void checkName(final Object value) {
		EList<NewElementRules> newElementRules = this.model.getNewElementRules();
		for (NewElementRules newElementRules2 : newElementRules) {
			if (newElementRules2.getName().equals(value)) {
				RuleEditPlugin.getPlugin();
				updateStatus(new Status(IStatus.ERROR, Implementation.PLUGIN_ID,
						"Rule already exists"));
				return;
			}
		}
		...
		for (NewElementRules newElementRules2 : newElementRules) {
			if (newElementRules2.getName().equals(this.combo.getText())) {
				...
				for (RemusTransferType remusTransferType : transferTypes) {
					this.newElement.getTransferTypes().add(
							(RemusTransferType) EcoreUtil.copy(remusTransferType));
				}
			}
		}

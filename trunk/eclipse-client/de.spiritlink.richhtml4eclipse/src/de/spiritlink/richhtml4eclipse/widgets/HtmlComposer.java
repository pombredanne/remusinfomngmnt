...
import de.spiritlink.richhtml4eclipse.Activator;
import de.spiritlink.richhtml4eclipse.internal.events.EventDataQueue;
import de.spiritlink.richhtml4eclipse.internal.events.EventTable;
...
//          System.out.println("STATUS CHANGED:" + event.text);
                ...
                if (splittedEventText[1].equals(START)) {   
                    if (eventId >= 0) {
                        if (HtmlComposer.this.eventDataQueue.getCurrentEvent() != null) {
                            HtmlComposer.this.eventTable.handleEvent(HtmlComposer.this.eventDataQueue.getCurrentEvent().type,
                                    HtmlComposer.this.eventDataQueue.getCurrentEvent());
                        }
                        HtmlComposer.this.eventDataQueue.startSequence(eventId);
                    } else {
                        HtmlComposer.this.eventDataQueue.addData(splittedEventText[0],
                                event.text.replaceFirst(splittedEventText[0] + SEPARATOR, "")); //$NON-NLS-1$
                    }
    ...
    private ProgressListener renderListener = new ProgressAdapter() {
        ...
        public void completed(ProgressEvent event) {
            ...
            for (final String commands : HtmlComposer.this.pendingCommands) {
                getShell().getDisplay().timerExec(HtmlComposer.this.delayAfterProgressCompleted,new Runnable() {
                    public void run() {
     ...
     * @param eventType the event. See {@link EventConstants}
    ...
    private static String resolveBaseHtml() {
        try {
            return FileLocator.resolve(Activator.getDefault().getBundle().getEntry(BASE_PATH)).toString();
        } catch (IOException e) {
    ...
    /**
     * Returns the HTML Content of the widget. Therefore a temporary listener
     * is registered to the widget that listens for the given html. Other registered
     * listeners that are listen for html will be also notified.
    ...
    /**
     * Sets a manual delay for executing pending commands. Commands can only be executed if
     * the tinyMCE and its JavaScript includes are fully loaded. The {@link ProgressListener} throws
     * an event if the page is loaded, but the includes may not be loaded. Therefore this method can be used
     * if you executing HTML manipulating commands against the widget and you see no result. The default is 50ms
     * which was enough in the tests. This fix was requested in 
     * {@link https://sourceforge.net/tracker/index.php?func=detail&aid=1679371&group_id=185136&atid=912061}
     * @param delayAfterProgressCompleted the delayAfterProgressCompleted to set
     */

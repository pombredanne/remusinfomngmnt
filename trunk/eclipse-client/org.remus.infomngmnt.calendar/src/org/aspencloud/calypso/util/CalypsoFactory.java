...
public class CalypsoFactory {
	...
	public static Task createTask(final String name, final Date start,
			final Date end) {
	...
	public static Task createTask(final String name, final Tasklist owner) {
		...
		try {
			...
			start.setDate(new Date(System.currentTimeMillis() + 5
					* TimeSpan.MINUTE));
			start.setAlarm(alarmStart);

			end.setName("End"); //$NON-NLS-1$
			end.setDate(new Date(start.getDate().getTime() + 15
					* TimeSpan.MINUTE));
			...
			if (!success) {
				...
				// } );
				System.out
						.println("FAILURE: createTask @ " + System.currentTimeMillis()); //$NON-NLS-1$
			}

...
public class TimeInterval {
	...
	public static final String[] FIELDS = new String[] { "year", //$NON-NLS-1$
			"month", //$NON-NLS-1$
			"week", //$NON-NLS-1$
			"day", //$NON-NLS-1$
			"hour", //$NON-NLS-1$
	...
	public static String format(TimeInterval interval) {
		...
		for (int i = 0; i < FIELDS.length; i++) {
			...
			if (val > 0) {
				...
				str += Integer.toString(interval.getField(i)) + " " + FIELDS[i]; //$NON-NLS-1$
				if (val > 1) {
					str += "s"; //$NON-NLS-1$
				}
			}
		}
		return (str.length() == 0) ? "immediately" : str; //$NON-NLS-1$
	}
	...
	public static TimeInterval parse(String string) {
		...
		Matcher matcher = Pattern.compile("\\b\\d+ \\w+\\b").matcher(string); //$NON-NLS-1$
		while (matcher.find()) {
			l.add(matcher.group());
		}

		for (Iterator i = l.iterator(); i.hasNext();) {
			String[] strs = ((String) i.next()).split(" "); //$NON-NLS-1$
			int value = Integer.parseInt(strs[0]);
			if (value > 1 && strs[1].endsWith("s")) { //$NON-NLS-1$
				strs[1] = strs[1].substring(0, strs[1].length() - 1);
			}
			int units = -1;
			Pattern pattern = Pattern.compile(strs[1] + ".*"); //$NON-NLS-1$
	...
	public TimeInterval(Date start, Date end) {
		...
		hours = tmpcal2.get(Calendar.HOUR_OF_DAY)
		...
		millis = tmpcal2.get(Calendar.MILLISECOND)
	...
	public TimeInterval(int years, int months, int days, int hours,
	...
	public TimeInterval(String initialValue) {
		...
		String[] strs = initialValue.split(":"); //$NON-NLS-1$
		int val;
		int i = 0;
	...
	private Date getDate(Date date, int dir) {
		...
		// add adjustments
		if (years > 0) {
			tmpcal.add(Calendar.YEAR, dir * years);
		}
		if (months > 0) {
			tmpcal.add(Calendar.MONTH, dir * months);
		}
		if (days > 0) {
			tmpcal.add(Calendar.DATE, dir * days);
		}
		if (hours > 0) {
			tmpcal.add(Calendar.HOUR_OF_DAY, dir * hours);
		}
		if (minutes > 0) {
			tmpcal.add(Calendar.MINUTE, dir * minutes);
		}
		if (seconds > 0) {
			tmpcal.add(Calendar.SECOND, dir * seconds);
		}
		if (millis > 0) {
			tmpcal.add(Calendar.MILLISECOND, dir * millis);
		}
		// set absolutes
		if (monthOfYear >= 0) {
			tmpcal.set(Calendar.MONTH, monthOfYear);
		}
		if (weekOfYear >= 0) {
			tmpcal.set(Calendar.WEEK_OF_YEAR, weekOfYear);
		}
		if (weekOfMonth >= 0) {
			tmpcal.set(Calendar.WEEK_OF_MONTH, weekOfMonth);
		}
		if (dayOfYear >= 0) {
			tmpcal.set(Calendar.DAY_OF_YEAR, dayOfMonth);
		}
		if (dayOfMonth >= 0) {
			tmpcal.set(Calendar.DAY_OF_MONTH, dayOfMonth);
		}
		if (dayOfWeek >= 0) {
		...
		if (minuteOfHour >= 0) {
			tmpcal.set(Calendar.MINUTE, minuteOfHour);
		}
		if (secondOfMinute >= 0) {
			tmpcal.set(Calendar.SECOND, secondOfMinute);
		}
		if (milliOfSecond >= 0) {
			tmpcal.set(Calendar.MILLISECOND, milliOfSecond);
		}
	...
	public void setPeriods(int repeat) {
		periods = (repeat < 0) ? FOREVER : repeat;
	}

	...
	private class CalendarLayout extends Layout {
		...
		protected Point computeSize(final Composite composite, final int wHint, final int hHint, final boolean flushCache) {
			...
			for(int i = 0; i < CCalendar.this.buttons.length; i++) {
				buttonHeight = Math.max(buttonHeight, CCalendar.this.buttons[i].computeSize(SWT.DEFAULT, SWT.DEFAULT).y);
			}

			Point size = new Point(
					Math.max(headerSize.x, Math.max(bodySize.x, footerSize.x)),
		...
		protected void layout(final Composite composite, final boolean flushCache) {
			...
			bodySize.y = r.height-(r.y+CCalendar.this.marginHeight+CCalendar.this.marginHeight+headerSize.y+buttonHeight+footerSize.y);
			
			CCalendar.this.header.setBounds(
			...
			CCalendar.this.body.setBounds(
					r.x+CCalendar.this.marginWidth,
					r.y+CCalendar.this.marginHeight+headerSize.y+buttonHeight,
					r.width-(r.x+CCalendar.this.marginWidth+CCalendar.this.marginWidth),
					bodySize.y
					);
			...
			CCalendar.this.footer.setBounds(
					r.x+CCalendar.this.marginWidth,
					r.y+CCalendar.this.marginHeight+headerSize.y+buttonHeight+bodySize.y,
					r.width-(r.x+CCalendar.this.marginWidth+CCalendar.this.marginWidth),
					footerSize.y
					);
...
//				previousWidth = r.width;
			...
			if((CCalendar.this.buttons.length > 0) && (CCalendar.this.buttons[0] != null) && !CCalendar.this.buttons[0].isDisposed()) {
				lblWidth = Math.max(lblWidth, CCalendar.this.buttons[0].computeSize(SWT.DEFAULT, SWT.DEFAULT).x);
			}
			...
			double dayWidth = (double) CCalendar.this.tcFigure.getTaskArea().width / (double) CCalendar.this.dates.length;

				if(CCalendar.this.buttons.length > 0) {
					CCalendar.this.buttons[0].setBounds(
							r.x+CCalendar.this.marginWidth,
							r.y+CCalendar.this.marginHeight+headerSize.y,
							lblWidth+1,
							buttonHeight
							);
					
					int buttonX = r.x+CCalendar.this.marginWidth+lblWidth+1;
					int buttonNextX = r.x+CCalendar.this.marginWidth+lblWidth+((int) dayWidth)+2;
					for(int i = 1; i < CCalendar.this.buttons.length; i++) {
						CCalendar.this.buttons[i].setBounds(
								buttonX,
								r.y+CCalendar.this.marginHeight+headerSize.y,
								buttonNextX-buttonX,
								buttonHeight
								);
						buttonX = r.x+CCalendar.this.marginWidth+lblWidth + ((int) (dayWidth*i)) + 2;
						buttonNextX = r.x+CCalendar.this.marginWidth+lblWidth + ((int) (dayWidth*(i+1))) + 2;
					}
				}
//			}
		}
...
//		});
	...
	public void propertyChange(final PropertyChangeEvent event) {
		if(RangeModel.PROPERTY_EXTENT.equals(event.getPropertyName())) {
			...
			((FigureCanvas) this.tcViewer.getControl()).getViewport().getVerticalRangeModel().removePropertyChangeListener(this);
		}
	}
	
	public Date getTopVisibleDate() {
		org.eclipse.draw2d.geometry.Point point = ((FigureCanvas) this.tcViewer.getControl()).getViewport().getViewLocation();
	...
	public void scrollToTime(final int hourOfDay, final int min, final int sec, final int milli) {
		...
		org.eclipse.draw2d.geometry.Point p = ((TasksCalendarPart) this.tcViewer.getContents()).getPointFromDate(tmpcal.getTime());
		if(p != null) {
			int vOffset = p.y;
			vOffset -= this.tcFigure.getTaskArea().y;//((tcFigure.getRowLabelHeight() / 2));
			((FigureCanvas) this.tcViewer.getControl()).scrollToY(vOffset);
		}
	}

	/**
	 * Adds a listener for selection-open to this CCalendar's viewers. Has no effect if an
...
//		data.horizontalSpan = 9;
	...
	private Composite createActivities(final Composite parent) {		
	    ...
	    ((FigureCanvas) this.acViewer.getControl()).setHorizontalScrollBarVisibility(FigureCanvas.NEVER);
	    ((FigureCanvas) this.acViewer.getControl()).setVerticalScrollBarVisibility(FigureCanvas.ALWAYS);
	...
	private Composite createTasks(final Composite parent) {		
	    ...
	    ((FigureCanvas) this.tcViewer.getControl()).setHorizontalScrollBarVisibility(FigureCanvas.NEVER);
	    ((FigureCanvas) this.tcViewer.getControl()).setVerticalScrollBarVisibility(FigureCanvas.ALWAYS);
	...
	private Composite createFooter() {
		...
		this.dayCal.setImage(PlatformUI.getWorkbench().getSharedImages().getImage(ISharedImages.IMG_OBJ_ELEMENT));
		...
		this.workWeekCal.setImage(PlatformUI.getWorkbench().getSharedImages().getImage(ISharedImages.IMG_OBJ_ELEMENT));
		...
		this.weekCal.setImage(PlatformUI.getWorkbench().getSharedImages().getImage(ISharedImages.IMG_OBJ_ELEMENT));
...
//	}
	...
	private boolean setDetailType(final int detailType) {
		...
		case TYPE_LIST:
			...
			break;
		case TYPE_MATRIX:
			if((detailType == TYPE_SINGLETON) || (detailType == TYPE_LIST) || (detailType == TYPE_MATRIX)) {
				this.detailType = detailType;
				result = true;
			}
			break;
		...
		for(int i = 0; i < this.buttons.length; i++) {
			if(this.buttons[i].getSelection()) {
				if(i == 0) {
					...
					if(setDetailDate(i-1) &&
							setDetailType(TYPE_SINGLETON)) {
						updateActivitiesCalendar();
						updateTasksCalendar();
						break;
					}
...
//		syncColumnWidths();
	...
	private void updateButtons() {
		// TODO: buttons always 1 > dates? (extra button is "week" button... "zoom" button?)
...
//			syncColumnWidths();
	...
	private void updateActivitiesCalendar() {
		...
		if(this.detailType == TYPE_SINGLETON) {
			...
			// don't bother shading a single day (they zoomed in on it for a reason didn't they?!?)
		} else {
			for(int i = 0; i < this.dates.length; i++) {
				tmpcal.setTime(this.dates[i]);
...
//		if((buttons.length > 0) && (buttons[0] != null) && !buttons[0].isDisposed()) {
...
//		spacers[1].setWidth((lblWidth - buttons[0].getWidth()) - ((lblWidth - buttons[0].getWidth()) / 2));
//		for(int i = 1; i < buttons.length; i++) {
//			spacers[2*i].setWidth((dayWidth - buttons[i].getWidth()) / 2);
//			spacers[(2*i)+1].setWidth((dayWidth - buttons[i].getWidth()) - ((dayWidth - buttons[i].getWidth()) / 2));
//		}
//
//		setRedraw(true);
//	}
	...
	public void zoom(final double x, final double y) {
		...
		PrecisionDimension d2 = new PrecisionDimension(((FigureCanvas) this.tcViewer.getControl()).getViewport().getSize());
		...
		int top = ((TasksCalendarPart) this.tcViewer.getContents()).getPointFromDate(tmpcal.getTime()).y;

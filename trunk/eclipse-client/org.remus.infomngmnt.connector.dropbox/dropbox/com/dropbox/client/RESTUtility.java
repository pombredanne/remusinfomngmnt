...
public class RESTUtility {
		...
		if (response.getStatusLine().getStatusCode() != 200) {
			...
			if (body.equals("OK")) {
				...
				try {
					...
					Map hm = new HashMap();
					StatusLine strangeError = new BasicStatusLine(
							new ProtocolVersion("HTTP", 1, 1), 999,
							"Bad JSON From Server: " + e.toString());
	 ...
	 */
	@SuppressWarnings("unchecked")
	static public Object request(String method, String protocol, String host,
			int port, String url, int apiVersion, Object params[],
			Authenticator auth) throws DropboxException {
		...
		try {
			if (method.equals("GET")) {
				target = buildFullURL(protocol, host, port,
						buildURL(url, apiVersion, params));
				req = new HttpGet(target);
			} else {
				target = buildFullURL(protocol, host, port,
						buildURL(url, apiVersion, null));

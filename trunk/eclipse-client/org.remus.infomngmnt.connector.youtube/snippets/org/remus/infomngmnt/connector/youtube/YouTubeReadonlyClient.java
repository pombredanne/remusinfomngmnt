...
public class YouTubeReadonlyClient {
	 ...
	 */
	private static final BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(
			System.in));
	 ...
	 */
	private static void printStandardFeeds(final YouTubeService service) throws IOException,
			ServiceException {
	 ...
	 * 
	 * @param feedTitle
	 *            - the title to print underlined
	 */
	private static void printUnderlined(final String feedTitle) {
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void printVideoEntry(final String prefix, final VideoEntry videoEntry,
			final boolean showCommentsAndResponses) throws IOException, ServiceException {
	 ...
	 * 
	 * @param videoEntry
	 *            the VideoEntry whose responses are to be printed
	 * @throws ServiceException
	 *             If the service is unable to handle the request.
	 * @throws IOException
	 *             error sending request or reading the feed.
	 */
	private static void printResponsesFeed(final VideoEntry videoEntry) throws IOException,
			ServiceException {
		if (videoEntry.getVideoResponsesLink() != null) {
			String videoResponsesFeedUrl = videoEntry.getVideoResponsesLink().getHref();
			System.out.println();
			printVideoFeed((YouTubeService) videoEntry.getService(), videoResponsesFeedUrl, false);
		}
	}
	 ...
	 * 
	 * @param videoEntry
	 *            the VideoEntry whose comments are to be printed
	 * @throws ServiceException
	 *             If the service is unable to handle the request.
	 * @throws IOException
	 *             error sending request or reading the feed.
	 */
	private static void printCommentsFeed(final VideoEntry videoEntry) throws IOException,
			ServiceException {
		Comments comments = videoEntry.getComments();
		if (comments != null && comments.getFeedLink() != null) {
			System.out.println("\tComments:");
			printFeed(videoEntry.getService(), comments.getFeedLink().getHref(), "Comment");
		}
	}

	/**
	 * Prints a basic feed, such as the comments or responses feed, retrieved
	 * from a feedUrl.
	 * 
	 * @param service
	 *            a YouTubeService object
	 ...
	 *            the url of the feed
	 * @param prefix
	 *            a prefix string to be printed in front of each entry field
	 * @throws ServiceException
	 *             If the service is unable to handle the request.
	 * @throws IOException
	 *             error sending request or reading the feed.
	 */
	private static void printFeed(final Service service, final String feedUrl, final String prefix)
	 ...
	 * 
	 * @param entry
	 *            the entry to be printed
	 * @param prefix
	 *            a prefix to be printed before each entry attribute
	 */
	private static void printEntry(final Entry entry, final String prefix) {
		System.out.println("\t\t" + prefix + " Title: " + entry.getTitle().getPlainText());
		if (entry.getContent() != null) {
			TextContent content = (TextContent) entry.getContent();
			System.out
					.println("\t\t" + prefix + " Content: " + content.getContent().getPlainText());
		}
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void printPlaylistEntry(final String prefix,
			final PlaylistLinkEntry playlistLinkEntry, final boolean showPlaylistContents)
			throws IOException, ServiceException {
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void printPlaylist(final Service service, final String playlistUrl)
			throws IOException, ServiceException {
	 ...
	 * 
	 * @param feedLinkEntry
	 *            the FeedLinkEntry to be printed
	 */
	private static void printFeedLinkEntry(final FeedLinkEntry feedLinkEntry) {
	 ...
	 * 
	 * @param subEntry
	 *            the SubscriptionEntry to be printed
	 */
	private static void printSubscriptionEntry(final SubscriptionEntry subEntry) {
	...
	private static void printUsage() {
		...
		System.out.println("\t6) Show Comments and Responses Feed for a single Video");
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void printUserProfile(final YouTubeService service) throws IOException,
			ServiceException {
		String testUser = promptForUser();
		printUnderlined("User Profile for '" + testUser + "'");
		UserProfileEntry userProfileEntry = service.getEntry(new URL(USER_FEED_PREFIX + testUser),
	 ...
	 *            a YouTubeService object.
	 * @param showPlaylistContents
	 *            if true, print only the first playlist, followed by all of its
	 *            contained entries
	 * @throws ServiceException
	 *             If the service is unable to handle the request.
	 * @throws IOException
	 *             error sending request or reading the feed.
	 */
	private static void printPlaylists(final YouTubeService service,
			final boolean showPlaylistContents) throws IOException, ServiceException {
		String testUser = promptForUser();
		printPlaylistsFeed(service, USER_FEED_PREFIX + testUser + PLAYLISTS_FEED_SUFFIX,
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void printSubscriptions(final YouTubeService service) throws IOException,
			ServiceException {
		String testUser = promptForUser();
		printSubscriptionsFeed(service, USER_FEED_PREFIX + testUser + SUBSCRIPTIONS_FEED_SUFFIX);
	}
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void printFavorites(final YouTubeService service) throws IOException,
			ServiceException {
		String testUser = promptForUser();
		printVideoFeed(service, USER_FEED_PREFIX + testUser + FAVORITES_FEED_SUFFIX, false);
	}
	 ...
	 *            a YouTubeService object.
	 * @param showCommentsAndResponses
	 *            whether or not to just print the first entry, followed by
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void printUploads(final YouTubeService service,
			final boolean showCommentsAndResponses) throws IOException, ServiceException {
		String testUser = promptForUser();
	 ...
	 *             if unable to read a line from standard input
	 */
	 ...
	 * @param showCommentsAndResponses
	 *            true if the user should be prompted for a video whose comments
	 *            and responses should printed
	 * @throws ServiceException
	 *             If the service is unable to handle the request.
	 * @throws IOException
	 *             error sending request or reading the feed.
	 */
	private static void printVideoFeed(final YouTubeService service, final String feedUrl,
			final boolean showCommentsAndResponses) throws IOException, ServiceException {
	 ...
	 *            the url of the feed
	 * @param showPlaylistContents
	 *            true if only one entry should be shown, followed by the
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void printPlaylistsFeed(final YouTubeService service, final String feedUrl,
			final boolean showPlaylistContents) throws IOException, ServiceException {
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void printSubscriptionsFeed(final YouTubeService service, final String feedUrl)
			throws IOException, ServiceException {
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void searchFeed(final YouTubeService service) throws IOException,
			ServiceException {
	 ...
	 *             error sending request or reading the feed.
	 */
	private static void searchFeedWithKeywords(final YouTubeService service) throws IOException,
			ServiceException {
		YouTubeQuery query = new YouTubeQuery(new URL(VIDEOS_FEED));
		...
		// a category filter holds a collection of categories to limit the
		// search
		...
		do {
			...
			Category category = new Category(YouTubeNamespace.KEYWORD_SCHEME, keywordTerm);
		...
		while (true) {
			try {
				...
				case 2:
					...
					break;
				case 3:
					// Searches the VIDEO_FEED for user supplied category
					// keyword terms.
				...
				case 6:
					...
					// feed
					// for that username as a numbered list. The user is asked
					// to choose
					// the number of a video entry for which the comments and
					// responses
				...
				case 8:
					...
					// playlists feed. The user is then asked to choose the
					// number of

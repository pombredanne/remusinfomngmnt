...
public class FileEditPage extends AbstractInformationFormPage {
	...
	protected void renderPage(IManagedForm managedForm) {
		...
		final GridData gd_generalSection = new GridData(SWT.FILL, SWT.CENTER,
				true, false);
		generalSection.setLayoutData(gd_generalSection);
		generalSection.setText(Messages.FileEditPage_General);

		final Composite composite = toolkit.createComposite(generalSection,
				SWT.NONE);
		...
		final Hyperlink openImageWithExternalApp = toolkit.createHyperlink(
				composite, Messages.FileEditPage_OpenWithExternal, SWT.NONE);
		openImageWithExternalApp.setLayoutData(new GridData(SWT.LEFT,
			...
			public void linkActivated(final HyperlinkEvent e) {
				...
				if (firstBinaryReferenceFile != null) {
					composite.getDisplay().asyncExec(new Runnable() {
						...
						public void run() {
							new OpenFileDialog(composite.getShell(),
									getModelObject()).open();
						}
					});
					Program.launch(firstBinaryReferenceFile.getLocation()
							.toOSString());
				}
			}
		});
		...
		final Hyperlink changeImageHyperlink = toolkit.createHyperlink(
				composite, Messages.FileEditPage_ChangeFile, SWT.NONE);
		changeImageHyperlink.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
				false, false, 4, 1));
			...
			public void linkActivated(final HyperlinkEvent e) {
				...
				if (open != null) {
					...
					try {
						...
						cc.append(CommandFactory.addFileToInfoUnit(newTmpFile,
								getModelObject(), getEditingDomain()));
						cc.append(SetCommand.create(getEditingDomain(),
								read.getChildByNodeId(Activator.FILENAME),
								read.getFeatureByNodeId(Activator.FILENAME),
								new Path(open).lastSegment()));
						cc.append(SetCommand.create(getEditingDomain(),
								read.getChildByNodeId(Activator.ORIGIN),
								read.getFeatureByNodeId(Activator.ORIGIN), open));
	...
	public void bindValuesToUi() {
		...
		bind(BindingWidgetFactory.createTextBinding(text, this),
				Activator.FILENAME);
	}

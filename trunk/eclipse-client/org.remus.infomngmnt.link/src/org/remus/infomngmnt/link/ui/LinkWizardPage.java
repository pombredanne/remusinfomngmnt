...
public class LinkWizardPage extends WizardPage {
	...
	public LinkWizardPage(InformationUnit newElement) {
		super("wizardPage");
	...
	public void createControl(Composite parent) {
		...
		makeContentSearchableButton.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				store.setValue(LinkPreferenceInitializer.INDEX_DOCUMENT,
						((Button) event.widget).getSelection());

			}

		});
		makeContentSearchableButton.setSelection(
				store.getBoolean(LinkPreferenceInitializer.INDEX_DOCUMENT));

		final Label theContentWillLabel = new Label(container, SWT.NONE);
		theContentWillLabel.setText("The content will be indexed and is avaialable for the local search.");
		...
		makeScreenhotOfButton.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event event) {
				store.setValue(LinkPreferenceInitializer.MAKE_SCREENSHOT,
						((Button) event.widget).getSelection());

			}

		});
		makeScreenhotOfButton.setSelection(
				store.getBoolean(LinkPreferenceInitializer.MAKE_SCREENSHOT));

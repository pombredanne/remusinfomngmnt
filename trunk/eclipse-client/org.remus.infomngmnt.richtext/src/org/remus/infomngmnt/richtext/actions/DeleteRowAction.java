 ...
 * 
 */
public class DeleteRowAction extends Action implements Listener {
	private HtmlComposer composer = null;

	public DeleteRowAction(final HtmlComposer composer) {
		super("", IAction.AS_PUSH_BUTTON);
		setToolTipText("Delete selected row(s)");
		setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(
				"de.spiritlink.richhtml4eclipse",
				"tiny_mce/jscripts/tiny_mce/plugins/table/images/table_delete_row.gif"));
		this.composer = composer;
		this.composer.addListener(EventConstants.DELETE_ROW, this);
	}
	...
	@Override
	public void run() {
		this.composer.execute(JavaScriptCommands.DELETE_ROW);
	}

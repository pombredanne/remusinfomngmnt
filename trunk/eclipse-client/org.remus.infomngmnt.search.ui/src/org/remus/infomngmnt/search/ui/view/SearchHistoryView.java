...
public class SearchHistoryView extends AbstractScrolledTitledView {
	...
	public void createViewContents(final Composite parent) {
		this.searchService = SearchUIActivator.getDefault().getServiceTracker().getService(
				ISearchService.class);
		this.editService = SearchUIActivator.getDefault().getServiceTracker().getService(
	...
	private class SearchMasterDetailsBlock extends MasterDetailsBlock {
		...
		protected void createMasterPart(final IManagedForm pManagedForm, final Composite parent) {
			...
			IObservableList observeList = EMFObservables.observeList(SearchActivator.getDefault()
			...
			this.viewer.setLabelProvider(new AdapterFactoryLabelProvider(
					SearchHistoryView.this.editService.getAdapterFactory()));
	...
	private class SearchHistoryDetails implements IDetailsPage {
		...
		public void createContents(final Composite parent) {
			...
			searchAgainText.setImage(HREF_SEARCH_AGAIN, ResourceManager.getPluginImage(
					SearchUIActivator.getDefault(), "icons/iconexperience/nav_refresh_blue.png"));
			...
			addToWatchedSearchesText.setImage(HREF_WATCH_SEARCH, ResourceManager.getPluginImage(
					SearchUIActivator.getDefault(), "icons/iconexperience/star_blue.png"));
			...
			HyperlinkAdapter adapter = new HyperlinkAdapter() {
				...
				public void linkActivated(final HyperlinkEvent e) {
					...
					if (HREF_WATCH_SEARCH.equals(href)) {
						...
						IFavoriteSearchHandler service = SearchActivator.getDefault()
					...
					if (HREF_SEARCH_AGAIN.equals(href)) {
						SearchHistoryView.this.searchService.search(
								SearchHistoryDetails.this.model, true, new ISearchCallBack() {
									...
									public void afterSearch(final IProgressMonitor monitor,
										...
										Display.getDefault().asyncExec(new Runnable() {
											public void run() {
												try {
													SearchUIActivator.getDefault()
															.getSearchContext().deactivate();
													PlatformUI.getWorkbench()
															.getActiveWorkbenchWindow()
															.getActivePage().openEditor(
																	new URIEditorInput(search
																			.eResource().getURI()),
																	SearchResultEditor.class
																			.getName());
												} catch (PartInitException e) {
													SearchUIActivator.getDefault().getLog().log(
															StatusCreator.newStatus(
																	"Error opening search-result",
																	e));
												}
											}
										});
	...
	@Override
	public void dispose() {
		SearchUIActivator.getDefault().getServiceTracker().ungetService(this.searchService);
		SearchUIActivator.getDefault().getServiceTracker().ungetService(this.editService);
		super.dispose();
	}

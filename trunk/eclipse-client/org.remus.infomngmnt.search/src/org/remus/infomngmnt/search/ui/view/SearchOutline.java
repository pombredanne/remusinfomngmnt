...
public class SearchOutline extends ContentOutlinePage {
	...
	public void createControl(Composite parent) {
		...
		this.toolkit.createLabel(composite, SearchResultEditor.SDF.format(new Date(Long.valueOf(this.model.getId()))), SWT.WRAP);
		...
		this.toolkit.createLabel(composite,this.model.getDateStart() != null ?  new SimpleDateFormat("yyyy-MM-dd").format(this.model.getDateStart()): "n.a.", SWT.WRAP);

		FormText labelStartEnd = this.toolkit.createFormText(composite, false);
		labelStartEnd.setText("<form><p><b>To:</b></p></form>", true, false);
		this.toolkit.createLabel(composite, this.model.getEndDate() != null ? new SimpleDateFormat("yyyy-MM-dd").format(this.model.getEndDate()): "n.a.", SWT.WRAP);

		FormText labelTypes = this.toolkit.createFormText(composite, false);
		labelTypes.setText("<form><p><b>Types:</b></p></form>", true, false);
		this.toolkit.createLabel(composite, StringUtils.join(this.model.getInfoType(),", "), SWT.WRAP);
		...
		//		Label createSeparator = this.toolkit.createSeparator(composite, SWT.HORIZONTAL);
		//		createSeparator.setLayoutData(twd);
		TableWrapData twoColumWrapData = new TableWrapData(TableWrapData.FILL_GRAB,TableWrapData.TOP,1,2);
		...
		final Section section2 = this.toolkit.createSection(this.container, Section.TITLE_BAR | Section.DESCRIPTION);
		section2.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true));
		section2.setText("Websearches");
		section2.setDescription("Use your search parameters for a online search through the following online-searvices...");
	...
	private void buildWebsearches(Composite composite2) {
		...
		for (Websearch websearch2 : websearch) {
			...
			sb.append("<a href=\"").append(StringEscapeUtils.escapeXml(
					NLS.bind(websearch2.getPattern(), this.model.getSearchString())));

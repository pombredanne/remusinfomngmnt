...
public class DefaultSpellChecker implements ISpellChecker {
	 ...
	 * @param word
	 *            The word to check
	 * @return <code>true</code> iff this word only contains upper-case letters,
	 *         <code>false</code> otherwise
	 */
	...
	/*
	 * @see
	 * org.eclipse.spelling.done.ISpellChecker#addDictionary(org.eclipse.spelling
	 * .done.ISpellDictionary)
	 */
	...
	public boolean acceptsWords() {
		...
		Set copy;
		synchronized (this.fDictionaries) {
	...
	/*
	 * @see
	 * com.onpositive.internal.ui.text.spelling.engine.ISpellChecker#addWord
	 * (java.lang.String)
	 */
	...
	/*
	 * @see
	 * org.eclipse.jdt.ui.text.spelling.engine.ISpellChecker#checkWord(java.
	 * lang.String)
	 */
	...
	/*
	 * @see
	 * org.eclipse.spelling.done.ISpellChecker#execute(org.eclipse.spelling.
	 * ISpellCheckTokenizer)
	 */
	public void execute(final ISpellEventListener listener,
			final ISpellCheckIterator iterator) {

		final boolean ignoreDigits = true;// fPreferences.getBoolean(
											// PreferenceConstants
											// .SPELLING_IGNORE_DIGITS);
		final boolean ignoreMixed = true;// fPreferences.getBoolean(
											// PreferenceConstants
											// .SPELLING_IGNORE_MIXED);
		final boolean ignoreSentence = true;// fPreferences.getBoolean(
											// PreferenceConstants
											// .SPELLING_IGNORE_SENTENCE);
		final boolean ignoreUpper = true;// fPreferences.getBoolean(
											// PreferenceConstants
											// .SPELLING_IGNORE_UPPER);
		final boolean ignoreURLS = true;// fPreferences.getBoolean(
										// PreferenceConstants
										// .SPELLING_IGNORE_URLS);
		final boolean ignoreNonLetters = true;// fPreferences.getBoolean(
												// PreferenceConstants
												// .SPELLING_IGNORE_NON_LETTERS
												// );
		final boolean ignoreSingleLetters = true;// fPreferences.getBoolean(
													// PreferenceConstants.
													// SPELLING_IGNORE_SINGLE_LETTERS
													// );
		final int problemsThreshold = 100;// PreferenceConstants.
											// getPreferenceStore
											// ().getInt(PreferenceConstants
											// .SPELLING_PROBLEMS_THRESHOLD);

		iterator.setIgnoreSingleLetters(ignoreSingleLetters);

		final Iterator iter = this.fDictionaries.iterator();
		while (iter.hasNext()) {
			((ISpellDictionary) iter.next())
					.setStripNonLetters(ignoreNonLetters);
		}
		...
		while ((problemCount <= problemsThreshold) && iterator.hasNext()) {
			...
			if (word != null) {
				...
				// reconciler
				if (!this.fIgnored.contains(word)) {

					starts = iterator.startsSentence();
					if (!this.isCorrect(word)) {

						final boolean isMixed = isMixedCase(word, true);
						final boolean isUpper = isUpperCase(word);
						final boolean isDigits = isDigits(word);
						final boolean isURL = isUrl(word);

						if ((!ignoreMixed && isMixed)
								|| (!ignoreUpper && isUpper)
								|| (!ignoreDigits && isDigits)
								|| (!ignoreURLS && isURL)
								|| !(isMixed || isUpper || isDigits || isURL)) {
							listener.handle(new SpellEvent(this, word, iterator
						...
						if (!ignoreSentence && starts
							...
							listener
									.handle(new SpellEvent(this, word, iterator
											.getBegin(), iterator.getEnd(),
											true, true));
							problemCount++;
						}
					}
	...
	/*
	 * @see
	 * org.eclipse.spelling.done.ISpellChecker#getProposals(java.lang.String
	 * ,boolean)
	 */
	...
	/*
	 * @see
	 * com.onpositive.internal.ui.text.spelling.engine.ISpellChecker#ignoreWord
	 * (java.lang.String)
	 */
	...
	/*
	 * @see
	 * com.onpositive.internal.ui.text.spelling.engine.ISpellChecker#isCorrect
	 * (java.lang.String)
	 */
	...
	/*
	 * @see
	 * org.eclipse.spelling.done.ISpellChecker#removeDictionary(org.eclipse.
	 * spelling.done.ISpellDictionary)
	 */
	...
	/*
	 * @see
	 * com.onpositive.internal.ui.text.spelling.engine.ISpellChecker#getLocale()
	 * 
	 * @since 3.3
	 */

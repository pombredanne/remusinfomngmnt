...
import org.remus.infomngmnt.ui.editors.internal.editpage.EditPage;
import org.remus.infomngmnt.ui.editors.internal.editpage.SourcePage;
 ...
 */
public class UIExtensionManager implements IEditorExtensionService {

	public static final String EXTENSION_POINT = EditorActivator.PLUGIN_ID + ".informationUi"; //$NON-NLS-1$
	...
	//
	public static final String OUTLINE_SECTION_NODE_NAME = "outlineSection"; //$NON-NLS-1$

	private Map<String, List<IEditPage>> items;

	private Map<String, List<ISourcePage>> sourcePage;

	private Map<String, IOutlineSection> outlineSections;

	// private Map<String, Map<String, AbstractCreationPreferencePage>>
	// preferencePages;
	...
	private void init() {
		...
		// FIXME
		// this.preferencePages = new HashMap<String, Map<String,
		...
		this.outlineSections = new HashMap<String, IOutlineSection>();
		...
		for (final IConfigurationElement configurationElement : configurationElements) {
			if (configurationElement.getName().equals(INFORMATION_NODE_NAME)) {
				...
				// FIXME
				// String transferId =
				// configurationElement.getAttribute(TRANSFER_ID_ATT);
				// String typeId =
				// configurationElement.getAttribute(TYPE_ID_ATT);
				...
				// }
				// Map<String, AbstractCreationPreferencePage> map =
				// this.preferencePages
				// .get(transferId);
				// if (map.get(typeId) == null) {
				// try {
				// map.put(typeId, (AbstractCreationPreferencePage)
			...
			} else if (configurationElement.getName().equals(OUTLINE_SECTION_NODE_NAME)) {
				try {
					this.outlineSections.put(configurationElement.getAttribute(ID_ATT),
							(IOutlineSection) configurationElement
	...
	// try {
	// return (ICreationTrigger)
	// this.creationTrigger.get(typeId).createExecutableExtension(
	// CLASS_ATT);
	// } catch (CoreException e) {
	// return null;
	// }
	...
	@Override
	public Collection<IOutlineSection> getOutlineSections() {
		List<IOutlineSection> values = new ArrayList<IOutlineSection>(this.outlineSections.values());
		Collections.sort(values, new Comparator<IOutlineSection>() {
			@Override
			public int compare(final IOutlineSection o1, final IOutlineSection o2) {
				return ((Integer) o1.getSortRanking()).compareTo(o2.getSortRanking());
			}
		});

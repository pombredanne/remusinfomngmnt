...
public class SwtFadeUtil {
	...
	public static FadeJob fastFadeIn(Shell shell, IFadeListener listener) {
		return new FadeJob(shell, 2 * FADE_IN_INCREMENT, FADE_RESCHEDULE_DELAY, 255, listener);
	}

	public static FadeJob fadeIn(Shell shell, IFadeListener listener) {
		return new FadeJob(shell, FADE_IN_INCREMENT, FADE_RESCHEDULE_DELAY,255, listener);
	}

	public static FadeJob fadeOut(Shell shell, IFadeListener listener) {
		return new FadeJob(shell, FADE_OUT_INCREMENT, FADE_RESCHEDULE_DELAY,-1, listener);
	}
	public static FadeJob fadeOut(Shell shell, int end, IFadeListener listener) {
		return new FadeJob(shell, FADE_OUT_INCREMENT, FADE_RESCHEDULE_DELAY,end, listener);
	}

	// TODO e3.4 get rid of reflection on 3.4 branch
	public static void fade(Shell shell, boolean fadeIn, int increment, int speed, int end) {
		try {
	...
	public static class FadeJob extends Job {
		...
		public FadeJob(Shell shell, int increment, long delay, int end, IFadeListener fadeListener) {
			super("Fading");
		...
		public void cancelAndWait(final boolean setAlpha) {
			...
			Display.getDefault().syncExec(new Runnable() {
				public void run() {
					if (setAlpha) {
						SwtFadeUtil.setAlpha(FadeJob.this.shell, getLastAlpha());
					}
				}
			});
		}
		...
		protected IStatus run(IProgressMonitor monitor) {
			...
			Display.getDefault().syncExec(new Runnable() {
				public void run() {
					...
					if (!SwtFadeUtil.setAlpha(FadeJob.this.shell, FadeJob.this.currentAlpha)) {
						// just in case it failed for some other reason than lack of support on the platform
						FadeJob.this.currentAlpha = getLastAlpha();
						SwtFadeUtil.setAlpha(FadeJob.this.shell, FadeJob.this.currentAlpha);
						FadeJob.this.stopped = true;
					}
			...
			if (this.currentAlpha == 0 || this.currentAlpha == 255
					|| this.currentAlpha <= this.end && this.increment < 0 || this.currentAlpha >= this.end && this.increment >0) {
				this.stopped = true;
			}

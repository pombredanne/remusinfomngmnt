...
public class InfoUnitSelectionDialog extends FilteredItemsSelectionDialog {
	...
	class ItemSelectionHistory extends SelectionHistory {
		...
		protected Object restoreItemFromMemento(final IMemento memento) {
			return ViewerActivator.getDefault().getApplicationService().getItemById(
					memento.getTextData(), null);
		}
	...
	protected void fillContentProvider(final AbstractContentProvider contentProvider,
		...
		Map<String, InformationUnitListItem> allItems = ViewerActivator.getDefault()
				.getApplicationService().getAllItems(progressMonitor);
	...
	protected IDialogSettings getDialogSettings() {
		IDialogSettings section = ViewerActivator.getDefault().getDialogSettings().getSection(
				SECTION_DIALOG);
		if (section == null) {
			ViewerActivator.getDefault().getDialogSettings().addNewSection(SECTION_DIALOG);
			section = ViewerActivator.getDefault().getDialogSettings().getSection(SECTION_DIALOG);
		}
		return section;
	}

...
public class NavigationDropAdapter extends EditingDomainViewerDropAdapter {
	...
	public void drop(final DropTargetEvent event) {
		...
		if (extractDragSource.size() == 0) {
			// FIXME
			// this.ruleByName = (NewElementRules)
			// EcoreUtil.copy(InfomngmntEditPlugin.getPlugin()
			// .getService(IRuleService.class).getRuleByName("Default Ruleset"));
			// List<RuleResult> process =
			// RuleProcessor.getInstance().process(event, this.ruleByName);
			// if (process.size() == 0) {
			...
			// CategoryUtil.categoryToString(targetCategory);
			// /*
			...
			// }
			// RuleValue predefCategory = (RuleValue)
	...
	//	
	// if (process.size() > 0) {
	// QuickAccessProvider[] provider = new
	// NewElementQuickAccessProider[process.size()];
